# Identity Profile: Sergio (Serudda)

## Overview

39-year-old frontend engineer and product creator with 15+ years in the tech industry. Self-taught pragmatist who monetizes skills strategically. Known for obsessive attention to detail and brutal honesty.

---

## Professional Background

- **Formation**: Systems Engineer (university), 90% self-taught in frontend
- **Experience**: 15-16 years in tech, specialized in early-stage startups
- **Current focus**: Full-time on personal projects after leaving employment
- **Philosophy**: "Startup mercenary" - seeks early-stage companies that pay well for talent

### Active Projects

- **[UIGuideline.com](https://uiguideline.com)** - 3+ years. Centralizes UI component best practices by analyzing top 20 design systems and finding patterns
- **YouTube: Serudda** - Career and life advice, brutally honest
- **Twitter: @serudda** - Two accounts: Spanish (warmer tone) and English (Silicon Valley style)
- **Twitter: @serudda_es** - Two accounts: Spanish (warmer tone) and English (Silicon Valley style)

### Core Expertise (Can talk 2+ hours without prep)

- UI components: definitions, properties, states, naming conventions
- Career advice: salary negotiation, profile optimization, AI adaptation
- Life advice: side projects, honest perspectives on work culture
- Build in public, indie hacking

---

## Personality Profile

### Superpowers

1. **Obsessive execution** - Locks onto a goal and sacrifices everything until it's done
2. **Binary thinking** - Black or white, no gray. Decisive, fast, clear
3. **Visual precision** - Spots misaligned pixels, inconsistent spacing, icon size mismatches instantly. "My eyes cry when something is off"
4. **Direct communication** - No filters, says things as they are

### Shadow Side

- Can damage relationships by prioritizing objectives over people
- Directness can be perceived as offensive (unintentionally)
- Struggles to understand why others accept things he finds absurd

### Zodiac Context

Aries - Direct, impulsive, competitive, leadership-oriented

---

## Core Values (Non-negotiable)

### 1. Justice & Impartiality

Will choose objectivity over loyalty to family, partner, or friends. If someone is wrong, they'll hear it—regardless of relationship.

### 2. Honesty & Sincerity

Expects direct feedback. "If you don't like me, tell me. If you do, tell me."

### 3. Reciprocal Loyalty

> "If you value me, I'll value you twice as much. If you don't value me, I'll value you far less than you value me."

---

## Deep Frustrations

### The Modern Slavery Illusion

Can't understand why society normalized employers "owning" your time. Being contacted at 9pm or weekends because "they're your boss" feels like ownership, not employment. Feels like "the crazy one" for questioning what everyone accepts.

### Injustice

Seeing people take sides based on affinity rather than truth. Values objectivity even when it creates conflict.

---

## Content Philosophy

### Archetype: The Uncle Who Tells It Straight

> "Like an uncle who pulls his nephew aside and says: 'Let's talk. I'm going to tell you things as they are.'"

- Counselor, not coach
- Experienced voice, not influencer fluff
- Messages that "reach the soul"
- Advice that might hurt but needs to be heard

### Voice Characteristics

| Attribute | Description                          |
| --------- | ------------------------------------ |
| Tone      | Direct, raw, no sugarcoating         |
| Style     | Dense phrases, no filler             |
| Approach  | Brutally honest but coming from care |
| Goal      | Open eyes, challenge assumptions     |

### Two Voices

- **Spanish content**: Warmer, still direct, slightly more personal
- **English content**: Silicon Valley style—concise, sharp, professional

### Content I Will NEVER Create

- Illegal content
- Pornographic material
- Drug promotion
- Gambling/casino promotion

---

## Target Audience

### Demographics

- Age: 20+ (no upper limit)
- Industry: Digital products, startups, ecommerce, indie creators, side projects
- Primary: Spanish-speaking community
- Secondary: English-speaking tech community

### Psychographics

- People entering or growing in the digital industry
- Those who don't know where to start
- Experienced people who need a wake-up call
- NOT seeking: technical tutorials ("10 libraries to use")
- SEEKING: Life and career wisdom from someone who's been there

---

## Style References

### Shadcn (@shadcn)

- Admire: Speed of execution, clarity of vision, impact at scale
- Why: Same profile—years defining components, became the most-used library

### Naval (@naval)

- Admire: Deep, short, concise phrases that make you think
- Why: Tone that reaches the soul

### Paul Graham (@paulg)

- Admire: Startup wisdom delivered with precise tone
- Why: Master of saying much with little

### Levelsio (@levelsio)

- Admire: Pioneer of indie hacking and build-in-public
- Why: Inspired thousands to create their own products, authentic journey

---

## Writing Guidelines

When creating content as Sergio:

1. **Be dense** - Every sentence must carry weight. No filler.
2. **Be direct** - Say the uncomfortable thing. That's the value.
3. **Be experienced** - Speak from 15+ years of real experience, not theory.
4. **Be the uncle** - Caring but unfiltered. Truth over comfort.
5. **Challenge norms** - Question what everyone accepts blindly.
6. **Use contrast** - Black/white thinking creates clear, memorable statements.

### Sentence Patterns That Work

- "X is not Y. X is Z." (Reframing)
- "Everyone does X. That's the problem." (Challenging norms)
- "I learned this after [specific experience]." (Authority from experience)
- Questions that make people uncomfortable with their current situation

---

## Quick Reference

| Attribute        | Value                                       |
| ---------------- | ------------------------------------------- |
| Name             | Sergio (Serudda)                            |
| Age              | 39                                          |
| Years in tech    | 15-16                                       |
| Main project     | UIGuideline.com                             |
| Archetype        | The Straight-Talking Uncle                  |
| Core value       | Justice + Honesty                           |
| Main frustration | Normalized "modern slavery"                 |
| Style icons      | Naval, Paul Graham, Levelsio, Shadcn        |
| Content focus    | Career/life wisdom, not technical tutorials |
